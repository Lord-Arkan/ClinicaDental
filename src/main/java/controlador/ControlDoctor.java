...
public class ControlDoctor extends HttpServlet {
    ...
    protected void agregarDoctor(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (action == null || action.isEmpty()) {
                ...
                Doctor doctorExistente = obj.buscarDoctorPorNombreApellidos(nombre, apellidos);
                if (doctorExistente != null) {
                    response.getWriter().write("{\"status\":\"error\",\"message\":\"El doctor ya está registrado\"}");
                    return;
                }
                ...
                if (registrado) {
                    response.getWriter().write("{\"status\":\"success\",\"message\":\"Doctor registrado correctamente\"}");
                } else {
                    response.getWriter().write("{\"status\":\"error\",\"message\":\"Error al registrar el doctor\"}");
                }
            }
   ...
   protected void agregarEspecialidad(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (obj2.especialidadExiste(tipo)) {
                // Si ya existe, mostrar mensaje de error con SweetAlert
                response.sendRedirect("ControlDoctor?opc=1&modalOpen=true&error=La especialidad ya existe");
                return;  // Salir del método si la especialidad ya existe
            }
